function AnnotationBrowsingCtrl($scope, $sce, $http) {
	...
	$scope.search = function(page) {
		...
		var text = ($scope.searchText!=undefined)?'&text=' + escapeRegExp($scope.searchText):''
	    ...
	    $http({method: 'GET', url: '/protected/searchAnnotation?max=' + $scope.paginationMax + '&offset=' + $scope.paginationOffset  + text + sources + permissions + inclusions + motivations + '&outCmd=frame'}).
		...
		console.log("search [page:" + (page?page:0) + ", max:" + $scope.paginationMax + "]")
	};
